...
import { Animated, Image as ImageNative, StyleSheet, View, TouchableOpacity, ActivityIndicator, Text, } from 'react-native';
...
export default function TransitionImage(props) {
    ...
    const { onPress, onLongPress, Component = onPress || onLongPress ? TouchableOpacity : View, placeholderStyle, PlaceholderContent, containerStyle, childrenContainerStyle = null, style = {}, ImageComponent = ImageNative, children, disabled = false, ...attributes } = props;
    const hasImage = Boolean(attributes.source);
    const { width, height, ...styleProps } = StyleSheet.flatten(style);
    const PlaceholderContentLoading = (<View style={styles.errorContainer}>
      <ActivityIndicator />
      <Text style={{ color: '#fff', marginTop: 8 }}>加载中...</Text>
    </View>);
    const PlaceholderContentError = (<View style={styles.errorContainer}>
      <Icon name="circle-close-o" color="#fff" size={24}/>
      <Text style={{ color: '#fff', marginTop: 8 }}>加载失败</Text>
    </View>);
    return (<Component onPress={onPress} onLongPress={onLongPress} accessibilityIgnoresInvertColors={true} style={StyleSheet.flatten([styles.container, containerStyle])} disabled={disabled || state.isLoading || state.hasError}>
      <ImageComponent testID="RNE__Image" transition={true} transitionDuration={360} {...attributes} onLoad={onLoad} onError={onError} style={StyleSheet.flatten([
