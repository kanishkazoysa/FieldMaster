...
import { StyleSheet, TouchableOpacity, Dimensions, View, Platform, // 动画
UIManager, // 动画
Animated, // 动画
PanResponder, // 手指拖拽
 } from 'react-native';
...
function DragDrawer(props) {
    ...
    const panResponder = useRef(PanResponder.create({
        ...
        onPanResponderMove: (evt, gestureState) => {
            return Animated.event([null, { dy: animatedViewHeight }], {
                listener: () => {
    ...
    const IconCustom = (icon) => {
        if (icon) {
            return (<>
          {typeof icon === 'string' ? (<Icon name={icon} size={25} color={theme.colors.primary_background || '#3578e5'}/>) : (icon)}
        </>);
        }
        else {
    ...
    return (<Animated.View style={StyleSheet.flatten([styles.drawer, dynamicDrawerStyles, style, { height: animatedViewHeight }])}>
