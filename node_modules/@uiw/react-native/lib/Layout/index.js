...
const Body = (props) => {
    ...
    return (<ScrollView style={[styles.body, style]} automaticallyAdjustContentInsets={false} {...other}>
      {children}
    </ScrollView>);
};
const Footer = (props) => {
    const { children, copyright, style, isShowCopyRight, ...other } = props;
    return (<View style={[styles.footer, style]} {...other}>
...
const Card = (props) => {
    ...
    return (<View style={style} {...other}>
      {extra ? (<View style={[styles.extra, styles.cardTitle]}>
          {showTitle && (<View style={{ flex: 2 }}>
              <Text style={[titleStyle]}>{title}</Text>
            </View>)}
          <View>{extra}</View>
        </View>) : (showTitle && <Text style={titleStyle ? [styles.cardTitle, titleStyle] : [styles.cardTitle]}>{title}</Text>)}
      {children && <View style={bodyStyle ? [styles.cardBody, bodyStyle] : [styles.cardBody]}>{children}</View>}
    </View>);
};
export const Container = (props) => {
    const { children, ...others } = props;
    return (<SafeAreaView style={{ backgroundColor: '#ededed' }} {...others}>
