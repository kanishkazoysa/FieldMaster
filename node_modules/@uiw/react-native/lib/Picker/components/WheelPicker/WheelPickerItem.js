...
const opacityFunction = (val) => 1 / (1 + Math.abs(val));
const scaleFunction = (val) => 1 - 0.1 * Math.abs(val);
const rotationFunction = (val) => 20 * val;
function WheelPickerItem({ textStyle, style, visibleRest, height, option, index, currentIndex }) {
    ...
    return (<Animated.View style={[styles.option, style, { height, opacity, transform: [{ rotateX }, { scale }] }]}>
