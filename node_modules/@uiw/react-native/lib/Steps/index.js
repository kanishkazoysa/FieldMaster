...
export default (props) => {
    ...
    const onStepsPress = (index) => {
        // onChange?.(index);
        onChange && onChange(current);
    };
    return (<View style={styles.steps} {...others}>
      {items.map((item, index) => {
            if (item.render) {
                return (<TouchableOpacity style={[styles.item]} key={index} onPress={() => onStepsPress(index)}>
            ...
            <View style={styles.wrap}>
              ...
              <View style={[
                    styles.circular,
                    {
                        backgroundColor: current >= index && !item?.status
                ...
                {item?.status === 'success' && (<Icon name="circle-check" size={22} fill={theme.colors.primary_background || '#3578e5'}/>)}
...
function createStyles({ bgColor, cirColor }) {
