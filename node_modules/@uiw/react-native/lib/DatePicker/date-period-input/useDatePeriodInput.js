...
export default function useDatePeriodInput({ value, onChange, format, }) {
    const [currentIndex, setCurrentIndex] = useSafeState(0);
    const [dates, setDates] = useSafeState(value ?? [undefined, undefined]);
    const [visible, { setTrue, setFalse }] = useBoolean(false);
    const [minDate, setMinDate] = useSafeState(undefined); // 对第二个日期输入框来说，它的最小值就是第一个日期输入框的值
    const [maxDate, setMaxDate] = useSafeState(undefined); // 对第一个日期输入框来说，它的最大值就是第二个日期输入框的值
