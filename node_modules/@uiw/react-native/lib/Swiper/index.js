...
const Swiper = (porps) => {
    ...
    const { dataSource = [], width = gitwidth, height = 130, time = 6000, autoplay = true, borderRadius = 0, dotStyle = 'dot', loading = false, index = 0, } = porps;
    ...
    return (<Loader loading={loading} rounded={5} maskColor="transparent">
      <View style={StyleSheet.flatten([styles.banner, { width, height }])}>
        <ScrollView ref={scrollToRef} horizontal={true} showsHorizontalScrollIndicator={false} pagingEnabled={true} onScrollBeginDrag={onScrollBeginDrag} onScrollEndDrag={onScrollEndDrag} onMomentumScrollEnd={onMomentumScrollEnd} onContentSizeChange={onContentSizeChange}>
          {[...dataSource, dataSource[0]].map((item, index) => {
            return (<View key={index} style={{ width, height }}>
                <View style={{ padding: 12 }}>
                  <TouchableOpacity activeOpacity={1} onPress={() => {
                    item.onClick && item.onClick();
                }}>
                    <Image key={index} style={StyleSheet.flatten([{ borderRadius, width: '100%', height: '100%' }])} resizeMode="cover" source={typeof item.url === 'number' ? item.url : { uri: item.url }}/>
                  </TouchableOpacity>
                </View>
              </View>);
        })}
        ...
        <View style={styles.dotBox}>
          {dataSource.map((_, index) => {
            return (<TouchableOpacity onPress={onClickDot.bind(this, index)} key={index} style={StyleSheet.flatten([
