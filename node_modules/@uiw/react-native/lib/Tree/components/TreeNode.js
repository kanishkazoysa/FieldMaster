...
const TreeNode = (props) => {
    ...
    const { icon: customIcon, level, disabled, checkable, expanded = false, label, checked = false, data, showIcon, } = props;
    const { style, handlerCheck, onClick } = useTreeNode(props);
    const iconRender = (checked) => {
        if (customIcon) {
            return customIcon(checked);
        }
        return (<Icon size={16} name={checked ? 'circle-check' : 'circle-o'} color={checked ? theme.colors.primary200 : theme.colors.gray300}/>);
    };
    return (<Animated.View style={[{ overflow: 'hidden' }, style()]}>
